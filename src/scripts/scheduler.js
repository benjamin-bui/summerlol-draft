...
function getLastSyncTime(db) {
  const row = db.prepare('SELECT value FROM sync_state WHERE key = ?').get(SYNC_STATE_KEY);
...
async function runIfDue(db, intervalDays, runFullSyncFn = runFullSync) {
  ...
  console.log(`[scheduler] ${intervalDays}-day interval elapsed (or first run) — running automatic Riot sync...`);
  try {
...
function startPeriodicSync(
  db,
  { intervalDays = DEFAULT_INTERVAL_DAYS, checkIntervalMs = CHECK_INTERVAL_MS, runFullSyncFn = runFullSync } = {}
) {
  if (!process.env.RIOT_API_KEY) {
    console.log('[scheduler] RIOT_API_KEY not set — automatic 14-day sync is disabled.');
    return null;
  }
  ...
  const handle = setInterval(() => runIfDue(db, intervalDays, runFullSyncFn), checkIntervalMs);
  // Don't let this timer keep the process alive on its own if everything
  // else has shut down.
  if (handle.unref) handle.unref();

  console.log(`[scheduler] automatic Riot sync scheduled every ${intervalDays} days.`);
  return handle;
}

module.exports = { startPeriodicSync, runIfDue, isDue, getLastSyncTime, setLastSyncTime };
