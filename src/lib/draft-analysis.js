...
function tournamentHasSignal(group, defaultConservativeRating) {
  return group.some(
    (p) => p.entryConservativeRating !== defaultConservativeRating,
  );
}
...
function computeDraftIQ(
  ...
  for (const [groupKey, group] of byTournament.entries()) {
    if (
      !tournamentHasSignal(
        group.map((p) => ({
          entryConservativeRating: p.entryConservativeRating,
        })),
        defaultConservativeRating,
      )
    )
    ...
    const rankedByEntry = [...group].sort(
      (a, b) => b.entryConservativeRating - a.entryConservativeRating,
    );
    const entryRankByKey = new Map(
      rankedByEntry.map((p, i) => [p.identityKey, i + 1]),
    );
    ...
    const rankedByExit = [...withExit].sort(
      (a, b) => b.exitConservativeRating - a.exitConservativeRating,
    );
    const exitRankByKey = new Map(
  ...
  const captainDraftIQ = [...byCaptain.entries()].map(
    ([captain, captainPicks]) => {
      const avgValue =
        captainPicks.reduce((sum, p) => sum + p.value, 0) / captainPicks.length;
      ...
      const bestLeaving = withLeaving.length
        ? [...withLeaving].sort((a, b) => b.leavingValue - a.leavingValue)[0]
        : null;
      const worstLeaving = withLeaving.length
        ? [...withLeaving].sort((a, b) => a.leavingValue - b.leavingValue)[0]
        : null;
      ...
      return {
        ...
        worstPick: worst
          ? {
              ...
              pickOrder: worst.pickOrder,
              entryRank: worst.entryRank,
              value: worst.value,
            }
          : null,
        bestPickLeaving: bestLeaving
          ? {
              displayName: bestLeaving.displayName,
              pickOrder: bestLeaving.pickOrder,
              exitRank: bestLeaving.exitRank,
              value: bestLeaving.leavingValue,
            }
          : null,
        worstPickLeaving: worstLeaving
          ? {
              displayName: worstLeaving.displayName,
              pickOrder: worstLeaving.pickOrder,
              exitRank: worstLeaving.exitRank,
              value: worstLeaving.leavingValue,
            }
          : null,
      };
    },
...
function computeTeamBalance(
  ...
  for (const row of draftRows) {
    ...
    const entry = entryLookup.get(
      `${row.identityKey}::${row.year}::${row.tournament}`,
    );
    if (!entry) continue;
    const groupKey = `${row.year}::${row.tournament}`;
  ...
  const tournamentsWithSignal = new Set(
    [...groupsByTournament.entries()]
      .filter(([, ratings]) =>
        ratings.some((r) => r !== defaultConservativeRating),
      )
      .map(([key]) => key),
  );
  ...
  const addMember = (
    ...
    if (!rosterStrength.has(key))
      rosterStrength.set(key, {
        members: [],
        seenKeys: new Set(),
    ...
    s.members.push({
      identityKey,
      displayName,
      conservativeRating: entry.conservativeRating,
    });
  };
  ...
  for (const row of draftRows) {
    ...
    addMember(
      ...
      captainKey,
      row.captain,
    ); // captain force-included, same as buildRosterMap
  }
  ...
  for (const g of games) {
    for (const [team, outcome] of [
      [
        g.team1,
        g.winner === "team1" ? "win" : g.winner === "draw" ? "draw" : "loss",
      ],
      [
        g.team2,
        g.winner === "team2" ? "win" : g.winner === "draw" ? "draw" : "loss",
      ],
    ]) {
      const key = `${g.year}::${g.tournament}::${team.key}`;
      if (!performance.has(key))
        performance.set(key, { wins: 0, losses: 0, draws: 0, games: 0 });
  ...
  const teams = [...rosterStrength.entries()].map(([key, strength]) => {
    ...
    const avgEntryRating = round3(
      strength.members.reduce((s, m) => s + m.conservativeRating, 0) /
        strength.members.length,
    );
    const winRate =
      perf && perf.games > 0 ? round3(perf.wins / perf.games) : null;
    return {
      year: parseInt(year, 10),
      tournament,
